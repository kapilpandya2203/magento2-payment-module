...
namespace Epay\Payment\Block\Adminhtml\Sales\Order\View;
...
class PaymentInfo extends \Magento\Backend\Block\Template
{
    ...
    public function getTransactionData()
    {
        try {
                    ...
                    if (!empty($transactionId)) {
                        ...
                        if (isset($transaction)) {
                            $result = $this->createEpayTransactionHtml(
                                ...
                                EpayConstants::REMOTE_INTERFACE,
                                $storeId
                            ) == 0) {
                            $result .= ' - ' . __(
            ...
            $result = __(
                "An error occurred while fetching the transaction information. Please see the ePay log file for more information."
            );
            $this->_epayLogger->addCommonError(
                $transactionId,
    ...
    public function getTransactionLogData()
    {
        try {
                ...
                if (isset($checkoutMethod)) {
                    ...
                    if (!empty($transactionId)) {
                        ...
                        if (isset($transactionOperations)) {
                            $result = '<tr><td colspan="4" class="epay_table_title">' . __(
                                    "Transaction History"
                                ) . '</td></tr>';
            ...
            $errorMessage = $ex->getMessage();
            $result = __(
                "An error occurred while fetching the transaction history. Please see the ePay log file for more information."
            );
            $this->_epayLogger->addCommonError(
                $transactionId,
     ...
     *
     * @param \Epay\Payment\Model\Api\Checkout\Response\Models\Transaction $transaction
     * @return string
     */
    public function createCheckoutTransactionHtml($transaction)
    {
        $res = '<tr><td colspan="2" class="epay_table_title">' . __(
                "ePay Online Checkout - Transaction information"
            ) . '</td></tr>';
        ...
        $authAmount = $this->_epayHelper->convertPriceFromMinorunits(
        ...
        $surchargeFee = $this->_epayHelper->convertPriceFromMinorunits(
        ...
        $capturedAmount = $this->_epayHelper->convertPriceFromMinorunits(
        ...
        $creditedAmount = $this->_epayHelper->convertPriceFromMinorunits(
     ...
     *
     * @param \Epay\Payment\Model\Api\Checkout\Response\Models\TransactionOperation[] $transactionOperations
    ...
    public function createCheckoutTransactionOperationsHtml(
        ...
        foreach ($transactionOperations as $operation) {
            ...
            if ($eventInfo['description'] != "") {
                $html .= '<tr class="epay_transaction_row_header">';
                ...
                $amount = $this->_epayHelper->convertPriceFromMinorunits(
                    $operation->amount,
                ...
                $html .= '</tr>';
                $html .= '<tr class="epay_transaction_description">';
                $eventInfoExtra = "";
     ...
     *
     * @param \Epay\Payment\Model\Api\Epay\Response\Models\TransactionInformationType $transactionInformation
    ...
    public function createEpayTransactionHtml($transactionInformation, $order)
    {
        ...
        $res = '<tr><td colspan="2" class="epay_table_title">' . __(
                "ePay - Transaction information"
            ) . '</td></tr>';

        $res .= '<tr><td>' . __("Transaction status") . ':</td>';
        $res .= '<td>' . $this->_epayHelper->translatePaymentStatus(
        ...
        $res .= '<tr><td>' . __("Surcharge fee") . ':</td>';
        $surchargeAmount = $this->_epayHelper->convertPriceFromMinorunits(
        ...
        if (isset($transactionHistoryInfo)) {
            $res .= '<br /><br />';
            $res .= '<tr><td colspan="2" class="epay_table_title epay_table_title_padding">' . __(
                    "History"
                ) . '</td></tr>';
    ...
    {
        $html = "";
        $html .= '<tr class="epay_table_history_tr"><td class="epay_table_history_td">' . str_replace(
                'T',
                ' ',
                $history->created
            ) . '</td>';
